...
export const findAllBreaksByParticipantNumber = async (
  participant_number: string
) => {
  const [rows, queryData] = await pool.query<RowDataPacket[]>(
    'SELECT b.id, b.session_id, b.has_accepted, b.duration, b.created_at FROM Breaks b LEFT JOIN Sessions s ON b.session_id = s.id WHERE s.participant_number = ?;',
    [participant_number]
  );
  console.log(rows);
  return rows;
